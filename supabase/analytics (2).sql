...
-- The display_name starts as a generated pseudonym. Admin edits replace it
-- with a human-friendly alias/name and subsequent presence/analytics writes
-- deliberately preserve that value.
...
begin
  ...
  values (
    ...
    coalesce(nullif(trim(p_app_version), ''), 'unknown')
  )
  on conflict (anonymous_id) do update set
    last_seen_at = greatest(public.convertlab_devices.last_seen_at, excluded.last_seen_at),
    last_calculation_at = case
      when excluded.last_calculation_at is null then public.convertlab_devices.last_calculation_at
      else greatest(
